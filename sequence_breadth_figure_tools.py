...
def jitter_plot_detections(detection_dict,threshold,plot_title,size=(10,6), vertical = False, by = 'status',pop_hues=None):
    ...
    if not vertical:
        if by == 'status':
            ...
            sns.stripplot(x='Condition', y='Value', data=exploded_df.reset_index(), hue='index', jitter=True,palette=pop_hues if pop_hues else None)
        ...
        sns.stripplot(x='Value', y='Condition', data=exploded_df.reset_index(), hue='index', jitter=True,palette=pop_hues if pop_hues else None)
...
def plot_detection_counts(prev_array_dict,threshold,plot_title,size = (10,6)):
    ...
    plt.figure(figsize=size)
    sns.barplot(x='Condition', y='Count', data=melted_df, hue='index', palette=pop_hues if pop_hues else None)
...
def plot_detection_freq(freq_dict,threshold,plot_title,size = (10,6),flip=False, pop_hues=None):
    freq_df = pd.DataFrame(freq_dict)
    melted_df = freq_df.reset_index().melt(id_vars='index', var_name='Condition', value_name='Frequency')
    sns.set_palette("colorblind")
    plt.figure(figsize=size)
    if flip:
        sns.barplot(x='Condition', y='Frequency', data=melted_df, hue='index', palette=pop_hues if pop_hues else None)
    else:
...
def box_plot_detections(detection_dict,threshold,plot_title,size=(10,6),pop_hues=None):
